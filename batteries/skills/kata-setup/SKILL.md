...
3. Check GitHub CLI: `gh auth status`
   - If authenticated, offer to create project labels:
     ...
     ```
4. Ensure CLAUDE.md references ceremony.md for workflow context:
   ...
   - If not, append this block:
     ...
     Read `batteries/ceremony.md` (resolved via `kata config get ceremony_path` or at the kata package root) for shared workflow instructions: commit patterns, PR creation, branch naming, environment checks, and test running.
     ```
